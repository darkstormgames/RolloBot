...
using RolloBot.Client.Helper.UserBot;
...
namespace RolloBot.Client.Tools.MK8DX
{
    public class AutoToad : ToolBase
    {
        ...
        public override ToolConfigBase Config => new AutoToadConfig(mainWindow);
        ...
        public override bool Start()
        {
            ...
            if (string.IsNullOrEmpty(userToken))
            {
                ...
                userBot = new UserBot(userToken);

            Task.Run(async () => await userBot.Start());
            
            Task.Run(async () =>
            {
                while (UserBot.DiscordClient == null)
                    await Task.Delay(5);

                UserBot.DiscordClient.Log += discordClient_Log;

                while (UserBot.Commands == null)
                    await Task.Delay(5);

                UserBot.Commands.AutoToadParameterCallback += commands_AutoToadParameterCallback;
        ...
        {
            UserBot.DiscordClient.LogoutAsync();
            this.userBot = null;
        ...
        private void commands_AutoToadParameterCallback(object sender, CallbackEventArgs e)
        {
            switch (e.Parameter)
            {
                case CallbackParameter.Started:
                    channelId = e.ChannelId;
                    isActivated = true;
                    mainWindow.AddLog(Client.LogType.Success, "Manual start of AutoToad successful");
                    break;
                case CallbackParameter.Paused:
                    isPaused = true;
                    mainWindow.AddLog(Client.LogType.Info, "AutoToad paused...");
                    break;
                case CallbackParameter.Resumed:
                    isPaused = false;
                    mainWindow.AddLog(Client.LogType.Info, "AutoToad resumed...");
                    break;
                case CallbackParameter.Stopped:
                    isActivated = false;
                    this.Stop();
        ...
        protected override void Cleanup()
        {
            ...
            UserBot.DiscordClient.Dispose();
            UserBot.Commands.Dispose();
        }
    }
}
